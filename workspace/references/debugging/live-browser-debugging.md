...
- MCP/SQL correlation after browser triage: use the normal APEX Admin Identity Gate from `workspace/SKILL.md`; the browser login does not authorize MCP database access.
...
When correlation uses live APEX SQL or APIs, apply the APEX Admin Identity Gate from `workspace/SKILL.md`. A browser session proves only browser authentication; it does not prove MCP database identity or permission to query APEX metadata.
...
Do not store screenshots, HAR files, console dumps, customer paths, request payloads, or exported logs in the skill tree. If the user wants a report, follow the external-output rules in `workspace/SKILL.md`.
