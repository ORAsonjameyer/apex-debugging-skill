...
Live MCP-backed APEX validation queries must use the confirmed APEX admin identity from `workspace/SKILL.md`. If validation crosses into generic database auditing, grants, runtime schemas, ORDS, or infrastructure checks, route that portion to the DB skill and use that skill's required connection/user.
