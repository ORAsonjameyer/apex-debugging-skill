...
- **Privileged SYSTEM APEX admin identity**: `SYSTEM` without `SYSDBA` may be used for APEX-admin-scoped work only after the exact uppercase `YES` confirmation required by `workspace/SKILL.md`.
