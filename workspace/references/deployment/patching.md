...
Live MCP-backed APEX patch-status checks in this skill require the confirmed APEX admin identity from `workspace/SKILL.md`. Privileged patch execution, database backup/restore, invalid-object repair, and ORDS runtime administration are DB/ORDS-skill work and must use those skills' required connection/user.
