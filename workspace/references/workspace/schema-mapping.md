...
MCP-backed APEX schema-mapping reads and `APEX_INSTANCE_ADMIN.ADD_SCHEMA` calls must use the confirmed APEX admin identity from `workspace/SKILL.md`. If schema privilege inspection, grants, quotas, user creation, or tablespace changes require broader database privileges, stop and route that portion to the DB skill with that skill's required connection/user.
