...
Live MCP-backed checks require the confirmed APEX admin identity from `workspace/SKILL.md`. Do not query APEX AI metadata, activity logs, debug logs, or APEX APIs under `SYS`, `SYSDBA`, app parsing schemas, workspace users, ORDS/APEX runtime users, generic deployment users, or unknown accounts. `SYSTEM` is allowed only after the exact uppercase `YES` confirmation required by the APEX Admin Identity Gate.
