...
- Do not create customer-specific output folders such as `workspace/<customer_name>`. Customer reports, logs, generated checklists, and derived artifacts must go to a user-confirmed external path outside the skill tree.
...
- APEXlang application or artifact work: do not perform it in this skill. Whenever an APEX application is created or changed with APEXlang, the APEXlang skill must be available and active. If debugging genuinely requires APEXlang-specific artifact inspection or changes, announce the handoff and route it to `apex/apexlang/SKILL.md`. Keep `workspace/` loaded only for workspace, deployment identity, connection safety, import/promotion, monitoring, and post-deploy validation context.
...
The companion skill domains are authoritative for work outside this skill:

- APEX application development and APEXlang artifacts: `apex/SKILL.md` and its routed references.
- Generic Oracle Database, SQL, PL/SQL, SQLcl, ORDS, security, performance, migration, and schema work: `db/SKILL.md` and its routed references.

During skill maintenance, inspect the corresponding `SKILL.md` in the sibling skills repository before adding guidance or tools. If the capability already exists there, reference and hand off to it; do not reimplement it in this skill. This applies to checks, scripts, APIs, database operations, debugging logic, deployment logic, and security controls.

Before using a companion skill for any action, announce the handoff and ask the user to confirm the switch. Do not silently combine identities, connections, permissions, protocol rules, or execution steps across skills. After confirmation, follow the selected skill's own `SKILL.md` and use its required user, connection, references, and safety gates.

Use these concise announcements:
...
The Workspace skill may retain only the APEX-specific context needed before or after the handoff, such as workspace identity, application target, import/promotion context, or APEX evidence correlation. It must not perform APEXlang work or duplicate the companion skill's implementation or checks. If the APEXlang skill is unavailable, stop before creating or changing an APEXlang application or artifact.

When a user asks to create, scaffold, generate, or materially change an APEX application or APEXlang artifact, stop before doing any artifact work and emit this handoff message:
...
```bash
node workspace/tools/apex-export-risk-scan.mjs --export <apex-export.sql> --format markdown
```

- Workspace admin checklist and supported APEX snippets:

```bash
node workspace/tools/workspace-admin-plan.mjs --action inventory --workspace <workspace-name>
```
