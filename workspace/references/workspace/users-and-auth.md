...
MCP-backed workspace-user reads or changes must use the confirmed APEX admin identity from `workspace/SKILL.md`. Do not create, unlock, reset, or inspect APEX workspace users under `SYS`, `SYSDBA`, parsing schemas, workspace users, ORDS/APEX runtime accounts, generic deployment users, or unknown accounts. `SYSTEM` is allowed only after the exact uppercase `YES` confirmation required by the APEX Admin Identity Gate.
