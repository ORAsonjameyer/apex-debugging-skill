...
Live APEX-side evidence gathering through MCP requires the confirmed APEX admin identity from `workspace/SKILL.md`: APEX dictionary queries, `APEX_WORKSPACE_ACTIVITY_LOG`, `APEX_DEBUG_MESSAGES`, supported APEX views, APEX debug/error logs, and APEX API checks must not run under `SYS`, `SYSDBA`, parsing schemas, workspace users, ORDS/APEX runtime users, generic deployment users, or unknown accounts. `SYSTEM` is allowed only after the exact uppercase `YES` confirmation required by the APEX Admin Identity Gate.
